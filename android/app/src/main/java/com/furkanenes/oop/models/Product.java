...
public abstract class Product {
	...
	public void addToComments(User u, int rating, String title, String comment) {
		...
		Comment inserted=new Comment(rating, u, title, comment, date,this);
	...
	public String display() {
		...
		o+=this.getBrand().getName().toLowerCase()+"*";
	...
	public float getPrice() {
		...
		if(this.campaigns.size()>0)
		{
			for(Campaign c: campaigns){
				currentPrice-=(currentPrice*c.getPercent()/100.0f);
			}
		}

		return Math.round(currentPrice * 100.0f)/100.0f;
	}
